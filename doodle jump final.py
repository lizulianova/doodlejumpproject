...
def load_image(name, colorkey=None):
    fullname = os.path.join('data final', name)
    try:
...
def finish_screen():
    ...
    global NCLOUD
    global lastcloud
    f = open("data final/" + 'record.txt', mode='r', encoding="utf-8")    
    lines = f.readlines()
    record = lines[0]
    f.close()
    ...
    if int(record) < NCLOUD:
        f = open("data final/" + 'record.txt', mode='w', encoding="utf-8")
        f.write(str(NCLOUD))
        f.close()
    run = True
    while run:
